...
export default function SearchScreen() {
  ...
  async function doSearch() {
    ...
    try {
      ...
      console.log('[search] doSearch results', { items: res.items?.length, page: res.page, totalPages: res.totalPages });
  ...
  async function loadMore() {
    ...
    console.log('[search] loadMore START', { page, totalPages });
    setIsSearching(true);
    setError('');
    try {
      const res = await searchPlayers(query.trim(), page + 1, 20);
      console.log('[search] loadMore fetched', { items: res.items?.length, page: res.page });
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.resultsSection}>
          ...
          {isSearching && results.length === 0 ? (
            ...
            (console.log('[search] rendering results count', results.length),
            results.map((p) => (
              <PlayerCard key={p.player_id + '-' + p.season} playerRecord={p} />
            )))
          )}
