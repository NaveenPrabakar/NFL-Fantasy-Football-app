...
def _render_receiving_yards(df: pl.DataFrame, player: PlayerSelection):
    ...
    title = f"{player.player_name} Regular Season Receiving Yards by Season"
    return build_line_figure(seasons, values, title, "Receiving Yards")
...
def build_wr_te_plots() -> list[PlotDefinition]:
    return [
        PlotDefinition(
            ...
            description="Regular season receiving yards by season.",
            renderer=_render_receiving_yards,
        ),
        PlotDefinition(
            position="wr-te",
