...
def _render_career_rushing_yards(df: pl.DataFrame, player: PlayerSelection):
    ...
    seasons = season_df.get_column("season").to_list()
    values = season_df.get_column(stat_column).to_list()
    title = f"{player.player_name} Career Rushing Yards by Season"
    return build_line_figure(seasons, values, title, "Career Rushing Yards")
...
def build_rb_plots() -> list[PlotDefinition]:
    return [
        ...
        PlotDefinition(
            position="rb",
            metric="career-rushing-yards",
            description="Career rushing yards by season.",
            renderer=_render_career_rushing_yards,
        ),
    ]
